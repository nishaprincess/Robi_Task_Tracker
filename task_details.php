...
$sti = oci_parse($conn, "SELECT  *FROM NISH69.NEWTASK WHERE TASKNAME='$subjectId' ");
...
while(($row = oci_fetch_array($sti,OCI_ASSOC+OCI_RETURN_NULLS))!=false){
 ...
 <label class="control-label"  for="username" style=" font-size: 18px;  display: block;
	...
	display:block;
	margin-top:2%;
	margin-bottom:0% width:15px; height:15px"href="http://localhost/ROBI%20TRACKER/assign_taskk.php?id='.$row["TASKID"].'">ACCEPT</a> 
	...
	display:block;
	margin-top:2%;
	margin-bottom:0% width:15px; height:15px"href="http://localhost/ROBI%20TRACKER/task_deny.php?id='.$row["TASKID"].'">DENY</a> 
